...
{
    [TestFixture]
    public class ArithmeticsFactoryTest
    {
        ...
        public void InvalidCreation() {
            Assert.Throws<InvalidOperationException>(() => ArithmeticsFactory.CreateAddOperation<ArithmeticsFactoryTest>());
        }

        [Test]
        public void DecimalArithmetics() {
            var theAdd = ArithmeticsFactory.CreateAddOperation<decimal>();
            var theSubtract = ArithmeticsFactory.CreateSubtractOperation<decimal>();
            var theMultiply = ArithmeticsFactory.CreateMultiplyOperation<decimal>();
            var theDivide = ArithmeticsFactory.CreateDivideOperation<decimal>();
            var theNegate = ArithmeticsFactory.CreateNegateOperation<decimal>();
            var theRound = ArithmeticsFactory.CreateRoundOperation<decimal>();

            Assert.That(ArithmeticsFactory.CreateZero<decimal>(), Is.EqualTo(0.0m));
            Assert.That(ArithmeticsFactory.CreateOne<decimal>(), Is.EqualTo(1.0m));
            Assert.That(ArithmeticsFactory.CreateMinValue<decimal>(), Is.EqualTo(decimal.MinValue));
            Assert.That(ArithmeticsFactory.CreateMaxValue<decimal>(), Is.EqualTo(decimal.MaxValue));
            ...
            var theNullableRound = ArithmeticsFactory.CreateRoundOperation<decimal?>();
        ...
        public void DoubleArithmetics() {
            var theAdd = ArithmeticsFactory.CreateAddOperation<double>();
            var theSubtract = ArithmeticsFactory.CreateSubtractOperation<double>();
            var theMultiply = ArithmeticsFactory.CreateMultiplyOperation<double>();
            var theDivide = ArithmeticsFactory.CreateDivideOperation<double>();
            var theNegate = ArithmeticsFactory.CreateNegateOperation<double>();
            var theRound = ArithmeticsFactory.CreateRoundOperation<double>();

            Assert.That(ArithmeticsFactory.CreateZero<double>(), Is.EqualTo(0.0));
            Assert.That(ArithmeticsFactory.CreateOne<double>(), Is.EqualTo(1.0));
            ...
            var theNullableRound = ArithmeticsFactory.CreateRoundOperation<double?>();
        ...
        public void FloatArithmetics() {
            var theAdd = ArithmeticsFactory.CreateAddOperation<float>();
            var theSubtract = ArithmeticsFactory.CreateSubtractOperation<float>();
            var theMultiply = ArithmeticsFactory.CreateMultiplyOperation<float>();
            var theDivide = ArithmeticsFactory.CreateDivideOperation<float>();
            var theNegate = ArithmeticsFactory.CreateNegateOperation<float>();

            Assert.That(ArithmeticsFactory.CreateZero<float>(), Is.EqualTo(0.0f));
            Assert.That(ArithmeticsFactory.CreateOne<float>(), Is.EqualTo(1.0f));
        ...
        public void IntegerArithmetics() {
            var theAdd = ArithmeticsFactory.CreateAddOperation<int>();
            var theSubtract = ArithmeticsFactory.CreateSubtractOperation<int>();
            var theMultiply = ArithmeticsFactory.CreateMultiplyOperation<int>();
            var theDivide = ArithmeticsFactory.CreateDivideOperation<int>();
            var theNegate = ArithmeticsFactory.CreateNegateOperation<int>();

            Assert.That(ArithmeticsFactory.CreateZero<int>(), Is.EqualTo(0));
            Assert.That(ArithmeticsFactory.CreateOne<int>(), Is.EqualTo(1));
        ...
        public void LongArithmetics() {
            var theAdd = ArithmeticsFactory.CreateAddOperation<long>();
            var theSubtract = ArithmeticsFactory.CreateSubtractOperation<long>();
            var theMultiply = ArithmeticsFactory.CreateMultiplyOperation<long>();
            var theDivide = ArithmeticsFactory.CreateDivideOperation<long>();
            var theNegate = ArithmeticsFactory.CreateNegateOperation<long>();

            Assert.That(ArithmeticsFactory.CreateZero<long>(), Is.EqualTo(0L));
            Assert.That(ArithmeticsFactory.CreateOne<long>(), Is.EqualTo(1L));
        ...
        public void DateTimeArithmetics() {
            var theAddTimeSpan = ArithmeticsFactory.CreateAddOperation<DateTime, TimeSpan, DateTime>();
            var theSubtractDateTime = ArithmeticsFactory.CreateSubtractOperation<DateTime, DateTime, TimeSpan>();
            var theSubtractTimeSpan = ArithmeticsFactory.CreateSubtractOperation<DateTime, TimeSpan, DateTime>();
            Assert.Throws<InvalidOperationException>(() => ArithmeticsFactory.CreateMultiplyOperation<DateTime>());
            Assert.Throws<InvalidOperationException>(() => ArithmeticsFactory.CreateDivideOperation<DateTime>());
            Assert.Throws<InvalidOperationException>(() => ArithmeticsFactory.CreateNegateOperation<DateTime>());
            var theNegateTimeSpan = ArithmeticsFactory.CreateNegateOperation<TimeSpan>();

            Assert.That(ArithmeticsFactory.CreateZero<TimeSpan>(), Is.EqualTo(TimeSpan.Zero));
            Assert.That(ArithmeticsFactory.CreateZero<DateTime>(), Is.EqualTo(new DateTime()));

            Assert.Throws<InvalidOperationException>(() => ArithmeticsFactory.CreateOne<DateTime>());
            Assert.Throws<InvalidOperationException>(() => ArithmeticsFactory.CreateOne<TimeSpan>());
        ...
        public void Exceptions() {
            Assert.Throws<InvalidOperationException>(() => ArithmeticsFactory.CreateMinValue<string>());
            Assert.Throws<InvalidOperationException>(() => ArithmeticsFactory.CreateMaxValue<string>());
        }
    }
}